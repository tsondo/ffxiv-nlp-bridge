...
from .minion_client import send_command
...
async def handle_command(user_cmd: UserCommand):
    ...
    try:
        minion_response = await send_command(command)
    except Exception as e:
        log.warning("Minion unreachable: %s", e)
        return CommandResponse(
            success=False,
            interpreted_as=command.task,
            command=command,
            error=f"Could not reach FFXIVMinion: {e}",
        )
    ...
    return CommandResponse(
        ...
        interpreted_as=command.task,
        command=command,
        minion_response=minion_response,
    )
